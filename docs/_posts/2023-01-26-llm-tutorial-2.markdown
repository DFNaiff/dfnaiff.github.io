...
For GPT models like ChatGPT, we can apply the RLHF framework by treating each user input $$s$$ and GPT output $$a$$ as a single episode with the trajectory $$(s, a)$$. In this case, the state corresponds to the user input, and the action is the model's output. By collecting contractor rankings based on these trajectories, we can create ChatGPT. In practice, additional techniques may be employed, such as having contractors rank multiple preferences simultaneously, pre-training using custom question/answer pairs, and including explicit warm-up text before the user interface (e.g., "hacking" the system). However, the essence remains the same.
